...
static char __Entry_Type_doc__[] =
    ...
    "Given an existing ACL:\n\n"
    "  >>> import posix1e\n"
    "  >>> acl = posix1e.ACL()\n\n"
    "An entry be created either by:\n"
    "\n"
    "  >>> e = posix1e.Entry(acl) # this creates a new entry in the ACL\n"
    "  >>> e = acl.append() # another way for doing the same thing\n"
    "\n"
    "or retrieved/iterated by:\n"
    "\n"
    "  >>> for entry in acl:\n"
    "  ...     print(entry)\n"
    "  ACL entry for undefined type\n"
    "  ACL entry for undefined type\n"
    "\n"
...
static char __Permset_Type_doc__[] =
    ...
    "\n"
    "The type exists only if the OS has full support for POSIX.1e.\n"
    "Given an existing ACL entry:\n\n"
    ">>> import posix1e\n"
    ">>> acl = posix1e.ACL()\n"
    ">>> entry = posix1e.Entry(acl)\n\n"
    "Its permset be retrieved either by either of the two following calls:\n\n"
    ">>> perms = entry.permset\n"
    "\n"
    "or:\n\n"
    ">>> perms = posix1e.Permset(entry)\n"
    "\n"
...
static char __posix1e_doc__[] =
    ...
    ">>> os.chmod(myfile.name, 0o0664)\n"
    ">>> acl1 = posix1e.ACL(file=myfile.name)\n"
    ">>> print(acl1)\n"
    "user::rw-\n"
    "group::rw-\n"
    "other::r--\n"
    ...
    "<BLANKLINE>\n"
    ">>>\n"
    ">>> b.applyto(myfile.name)\n"
    ">>> print(posix1e.ACL(file=myfile.name))\n"
    "user::r-x\n"
    "group::---\n"
    "other::---\n"
    "<BLANKLINE>\n"
